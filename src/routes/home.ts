...
export const home: RouteHandler = async (req, ctx) => {
  const countRow = await queryOne<{ n: number }>(
    "SELECT COUNT(*) AS n FROM submissions WHERE status = 'reviewed' AND repro_status = 'reproduced'",
  );
  ...
  const intro = h`
    <div class="home-top">
      ...
      <p>There ${total === 1 ? raw("is") : raw("are")}
        currently <strong>${total}</strong> active ${total === 1 ? raw("entry") : raw("entries")}.</p>
