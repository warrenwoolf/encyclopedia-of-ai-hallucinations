...
- **Server-rendered HTML.** No client framework. Two small vanilla-JS files: `src/static/theme.js` (dark-mode toggle) and `src/static/browse.js` (progressive enhancement for the browse filters — intercepts the filter form + browse links to apply filters without a full reload, swapping `#browse-root`). Both degrade gracefully (the browse sidebar is a plain GET form that works without JS). Don't add more client JS without a real reason — the *JS* surface is deliberately minimal (the CSS is allowed to be richer; see House style). **Static files are served from an explicit allowlist** in `src/server.ts` (`STATIC_FILES`) — adding a new `/static/*` file means adding it there AND to the `deploy.sh` Cloudflare purge list.
...
- **User email addresses are owner-only.** The `/admin/users` / `/admin/staff` roster hides the email column from non-owner staff (gated on `canManage` = `!!ctx.owner` in `renderTable`). Staff see usernames + verification state, not addresses.
...
**The current host is the SSH alias `randy` (a temporary box — Pi/homelab). Don't hardcode `randy` anywhere in source; `deploy.sh` is the one place it lives, so retargeting is a one-line edit there.** The public URL is `https://enaih.org` (the brand handle **ENAIH** = **EN**cyclopedia of **AI** **H**allucinations; the title/logo highlight those letters via `.acro`). The old `eah.warrenwoolf.com` 301-redirects to it via a Cloudflare Redirect Rule. Both are fronted by Cloudflare → cloudflared → `127.0.0.1:8090`. **Internal identifiers stay `EAH`** (the `eah_number` column, `eah-id.ts`, the `A######` system, the project shorthand in this file) — only *public-facing* copy uses ENAIH. **The cloudflared tunnel is dashboard-managed** (`cloudflared tunnel run` + token in `/etc/cloudflared/tunnel.env`, no local ingress file), so adding/changing public hostnames is done in Zero Trust → Networks → Tunnels → Public Hostnames, not in source.

⚠️ **Cloudflare caches `/static/*` aggressively (edge TTL set to 4h via a zone rule, longer than the `max-age=3600` we send).** After deploying a change to a static asset (`style.css`, `theme.js`, `browse.js`, `google.js`), the new file is live *in the container* immediately but the public URL keeps serving the stale edge copy — and `Ctrl+Shift+R` only busts the *browser* cache, not Cloudflare's. **`deploy.sh` now purges the static assets automatically** as its last step (runs **locally** on the deploying machine, after the remote build/migrate): it reads a Cloudflare API token + account id from local files (`~/Credentials/cloudflare-ww-purge-token.txt`, `~/Credentials/cloudflare-account-id.txt` — override via `CF_TOKEN_FILE`/`CF_ACCOUNT_ID_FILE`), resolves the **zone id** from the account id + `SITE_URL`'s registrable domain via the `/zones` API (the file is an *account* id, not a zone id — they differ), then POSTs the `/static/*` URLs to `purge_cache` (keep this list in sync with `STATIC_FILES` in `src/server.ts` when adding assets). The token needs *Zone → Cache Purge* **and** *Zone Read* (the latter for the zone lookup). **Since `SITE_URL` is now `https://enaih.org`, the purge resolves the `enaih.org` zone — the purge token must therefore grant those two permissions on the `enaih.org` zone** (an account-scoped token covering all zones is simplest; a token scoped only to the old `warrenwoolf.com` zone will silently skip the purge with a warning). If the cred files are missing or the zone can't be resolved, the purge is **skipped with a warning** and the deploy still succeeds (purge manually via dashboard → Caching → Purge). TypeScript/server changes are unaffected (never cached). Symptom of a missed purge: `curl -sI <url> | grep cf-cache-status` shows `HIT` with an old `last-modified`. There is deliberately no asset cache-buster — purge (now automated) is the mechanism.
...
The site is **not officially launched yet.** Two things are deliberately in a pre-launch posture and travel together — flip them in the **same** deploy when you launch:

- **`src/static/robots.txt` is `Disallow: /`** (whole site uncrawlable) — kept this way on purpose so the work-in-progress isn't indexed. To launch: change it to allow crawling (empty `Disallow:`), keeping the `Sitemap:` line.
- **The "IN DEVELOPMENT" banner** (`layout.ts`, gated on `config.inDevelopment` = the `IN_DEVELOPMENT` env flag, default `true`). It's an env flip, not a code edit — set `IN_DEVELOPMENT=false` in the prod `.env` on the host to remove it.

Don't lift `robots.txt` on its own — an indexed site that still shows a "work in progress" banner is the worst of both. (Reminder: changing `robots.txt` is a static-asset change, so the deploy purges it at the edge — see the Cloudflare cache note above.)
...
- **Restrained and OEIS-influenced, but modernizing — no longer strictly "spare."** The baseline is still serif body, plain forms, and simple tables, and the *motion* rules are unchanged: **no transitions, no animations, no hover-scale, no JS-driven visual effects.** What has been relaxed is the old "no fancy CSS" absolutism: the site is now full-bleed with a richer browse layout and styled entry-cards, and modern static CSS is welcome where it earns its keep. Specifically, the following are deliberate and **should not be "simplified" back out**:
  ...
  - **Centered-by-default content width.** Two variables: `--content-width` (the full-bleed bands — top bar, header, footer — default `100%`, always edge-to-edge) and `--main-width` (the `<main>` column, default `760px` centered, like the about page). The site default is a centered reading column; pages opt OUT to full-bleed by overriding `--main-width`: `body.browse-page` (browse + home) sets `100%`, `body.admin-wide` (staff tables) sets `min(1400px, 96vw)`. `body.text-page` is now redundant with the default but kept as an explicit marker on reading pages. Don't collapse the two vars back together — the bands must stay full-bleed even when `main` is centered.
  - **Outer page gutter** is the `--page-pad` variable (`50px` desktop, stepping to `24px` ≤900px and `12px` ≤600px). The top bar, title header, `main`, and `footer` all draw their horizontal padding from it, so the left/right screen margin is changed in one place. Don't reintroduce hardcoded `12px` horizontal paddings on those full-bleed bands.
  ...
  - Still off-limits: animation/transitions, client-side JS beyond `theme.js` + `browse.js` (browse-filter progressive enhancement), and CSS frameworks. The account dropdown is intentionally **pure CSS** (`:hover`/`:focus-within`), not JS. Reach for plain modern CSS, not a toolkit.
- **Shared listing UI: the entry-card.** The browse listing, the home page, and the `/my/submissions` dashboard all render submissions as the same `.entry-card` component — a collapsible `<details>` with a colored `.entry-card-head` summary, an `.entry-info` grid, and `.entry-field-box` prompt/response boxes. The long-text clamp helper `longField()` is **exported from `src/routes/browse.ts`** and reused by `my.ts`; the entry page (`/e/:id`) reuses `.entry-card-head` too. So a change to the card markup or its CSS (§8j) ripples across browse/home/dashboard/entry — grep `entry-card` before editing. Convention: the **Entry ID is the permalink**. On the browse/home cards it sits in the colored `.entry-card-head` *above* the title (`.entry-card-eid`), and **both the id and the title are `<a>`s to `/e/:id`** — clicking either navigates, clicking elsewhere on the head toggles the `<details>`. (The `/my/submissions` dashboard card keeps the Entry ID as the first row of the info grid, but its **title is an `<a>` to the overview page** `/my/submissions/:eahId` — *not* `/e/:id`, since drafts/pending aren't public — following the same click-title-navigates / click-head-toggles convention. Rejected rows have a freed A-number and no overview page, so their title falls back to a plain span.)
...
- **CSS gotcha — three theme blocks must stay in var-parity (§8a).** Colors are defined in *three* places: `:root` (light default), `@media (prefers-color-scheme: dark) :root:not([data-theme="light"])` (auto/system dark), and `:root[data-theme="dark"]` (explicit toggle). When you add or change a `--var` in one dark block, **change it in the other dark block too** — they are NOT generated from a shared source. A drift here is invisible under the explicit toggle and only shows on system-dark with no toggle set: e.g. `--entry-field-bg` was set in `:root[data-theme="dark"]` but missing from the auto-dark block, so `.entry-field-box` kept the light `#e8e8e8` under a light `--text` → near-invisible text for OS-dark users. Grep all three blocks for the var name before shipping a theme color change.
...
- Don't store raw IPs.
- Don't write client-side JS frameworks. The only client JS is `theme.js` (dark-mode toggle) and `browse.js` (browse-filter progressive enhancement) — both deliberate, framework-free, and gracefully degrading.
