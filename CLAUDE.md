...
- **Shared listing UI: the entry-card.** The browse listing, the home page, and the `/my/submissions` dashboard all render submissions as the same `.entry-card` component — a collapsible `<details>` with a colored `.entry-card-head` summary, an `.entry-info` grid, and `.entry-field-box` prompt/response boxes. The long-text clamp helper `longField()` is **exported from `src/routes/browse.ts`** and reused by `my.ts`; the entry page (`/e/:id`) reuses `.entry-card-head` too. So a change to the card markup or its CSS (§8j) ripples across browse/home/dashboard/entry — grep `entry-card` before editing. Convention: the **Entry ID is the permalink**. On the browse/home cards it sits in the colored `.entry-card-head` *above* the title (`.entry-card-eid`), and **both the id and the title are `<a>`s to `/e/:id`** — clicking either navigates, clicking elsewhere on the head toggles the `<details>`. (The `/my/submissions` dashboard card still keeps the Entry ID as the first row of the info grid.)
